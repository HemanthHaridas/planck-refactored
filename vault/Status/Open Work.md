...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **U1.4c landed. Rank-6 triples rel ~1.9e-3 (was 8.8e-2) and the fixture is exonerated.** ccgen's rank-6 T3 is cleared by its own closed-shell oracle (1.5e-12 vs ‖R‖~1e3, no PySCF). **Four PySCF-interface defects found and fixed**, each handing the two sides different amplitudes: (1) the re-antisymmetrization was unnormalized — PySCF's blocks arrive already antisymmetric so re-applying it *multiplied* them by 4× (t2aa) / 36× (t3aaa); the tell was ‖ref‖=1.7e-1 at converged amplitudes where PySCF's own residual is ~1e-10; (2) `t2aa` is determined by `t2ab`; (3) `t3aaa` by `t3aab`; (4) **a block carrying `aabaab`'s antisymmetries is not thereby a valid amplitude block** — the real block has exactly those two signed symmetries and no others (checked exhaustively), so no permutation test separates it from antisymmetrized noise; what does is that the two same-spin closure forms agree. Fixed by building the perturbation as a slice of a genuine antisymmetric spin-orbital tensor. **The two closure forms (3-term and 36-term/12) are EQUIVALENT on valid blocks** (both reproduce a genuine `aaaaaa` to ~2e-15) — an earlier claim that they were inequivalent and side-specific is retracted. Singles/doubles now exact (rel ~1e-14). **The residual ~0.2% survives a fixture in which every relation holds to ~1e-17, so it is a genuine ccgen-vs-PySCF disagreement about the t3-linear part of T3** — `expectedFailure`. Rank-4 UCC fully validated; do not block U2+ on this |
