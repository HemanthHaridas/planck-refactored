...
`CCGEN_UCC_RANK6_PYSCF_GAP_HANDOFF.md` is a **deliberate handoff**: its answered half is already
architecture-shaped, and it converts to `CCGEN_UCC_NUMERIC_VALIDATION.md` when the rank-6 PySCF gap
closes — at which point the three older UCC scope docs merge into it.) They should merge into **one** answer — they are two halves of a single question, split by
