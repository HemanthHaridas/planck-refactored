...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **U1.4c.1–c.3 landed; PySCF interface fixed. Rank-6 triples now rel 1.3e-3 (was 8.8e-2).** ccgen's rank-6 T3 is cleared by the closed-shell oracle (`U14c2RankSixClosedShellOracleTests`, no PySCF, triples 1.5e-12 vs ‖R‖~1e3). **Three interface defects were then found and fixed in the PySCF gate, all of which moved the REFERENCE off convergence**: (1) the re-antisymmetrization was unnormalized — PySCF's `t2aa`/`t3aaa` arrive already antisymmetric, so re-applying it *multiplied* them by **4× and 36×**; the tell was ‖ref‖=1.7e-1 at converged amplitudes where PySCF's own residual is ~1e-10; (2) `t2aa = t2ab − t2ab.transpose(0,1,3,2)` exactly, so `t2aa/bb` cannot be perturbed independently; (3) same for `t3aaa/bbb` via the U1.4c.1 closure. Now only t3 is perturbed (nothing derives t3 from a perturbed t2), and **singles/doubles are exact at rel ~1e-14**. The remaining triples gap is a **fixed multiplicative deficit in the t3-linear part** — constant at rel ~1.0e-3 across a 100× perturbation range, and 2.7e-11 at zero perturbation. The earlier 'additive, t3-independent' characterization was an artifact of defect (1) and is **withdrawn**. Rank-4 UCC fully validated; rank-6 triples still `expectedFailure` but the gap is 68× smaller and much better characterized. Do not block U2+ on it |
