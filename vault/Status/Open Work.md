...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **Rank-4 UCC fully validated (~6e-16). Rank-6 singles/doubles exact (~1e-15); `triples_aaaaaa` open.** Gate: `U14RankSixVsPyscfTests` (water/6-31g). Bisected on a deterministic probe: at `t1=t3=0` ccgen gives 4.93e-2 vs PySCF 3.55e-3 (~14×) while singles/doubles stay exact, and every ccgen term outside the pure-t2 class is 0. The 18 surviving `t2·v` terms are the standard `P(i/jk)P(a/bc)` expansions of two **textbook** T3 terms (9 `ooov`, 9 `ovvv`) — legitimate, not spurious; they cancel heavily (38× in the full residual) and dropping either family worsens agreement. **Dead hypotheses:** T1-dressing convention (`t1=0` changes nothing), a spurious term family, any integer over/under-count. **ccgen's own RCC manifold — a third source independent of both — reproduces `triples_aabaab` to 1.6e-17**, though only at converged amplitudes where ‖R‖~1e-10, so that clearance is not yet decisive. Landed en route: the t3 closure `t3_aaaaaa = (1/12)·𝒜_bra𝒜_ket t3_aabaab` (all 36 signed perms), cross-validated on the independent `bbb`←`bba` pair — necessary because the 36 permutation images span **rank 5**, so a least-squares fit alone proves nothing. **Next: the `t3_abbabb` relation** (`X ≠ M.transpose(2,1,0,5,4,3)`), which blocks a non-vacuous third-source comparison. **Rank-6 triples untrusted; rank-4 safe** |
