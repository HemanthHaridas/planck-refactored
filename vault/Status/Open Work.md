...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **Rank-4 UCC fully validated (~6e-16). Rank-6 singles/doubles exact (~1e-15); `triples_aaaaaa` open at ~1.1e-2. U1.4c.1 LANDED — U1.4c.2 unblocked.** Both rank-6 closed-shell closures are now DERIVED rather than fitted: `t3_aaaaaa` from `_split_same_spin_amplitude`, whose base reordering must be read as the **inverse** permutation on array axes (forward fails by the block's full magnitude; inverse is exact to 4.8e-18), and `t3_abbabb = bba.transpose(5,2,3,4,0,1)`, picked out by carrying abbabb's OWN antisymmetry (the beta pairs). **A previously committed FITTED closure was wrong and looked right** — the uniform-1/12 antisymmetrizer reproduced both real block pairs exactly yet differs from the derived form by ~80% on a generic block; the fixture's permutation images span rank 5 vs 9 generic, so the fit was underdetermined and the `bbb`←`bba` cross-check (shared null space) did not catch it. Four dead hypotheses for the triples itself: T1-dressing, a spurious term family (the 18 `t2·v` terms are textbook), any integer over/under-count, and the t3 blocks. **Next: U1.4c.2 — extend F2.3's oracle to rank 6 with perturbed amplitudes, then U1.4c.3 decides which side is wrong.** Do not resume the max-norm bisect (38× cancellation). **Rank-6 triples untrusted; rank-4 safe** |
