...
| `CCGEN_WIRING_THE_DERIVATION_ROUTE` | **W1-W2 LANDED (2026-08-26); W3-W5 open.** The follow-through from `CCGEN_TWO_DRESSING_ROUTES`: the derivation route is gated and worth 2-7x but has **no production caller**. **The technical blocker is gone** — `emit_factorized_from_equations` (W1) takes the equations instead of generating them, byte-identical over six configurations, and the spatial TU now **compiles** against the real CC headers merged and un-merged, 59 → 31 builders (W2). The emitter's ERI set is re-verified as exactly the four parity-`+1` relations, which matters now that merging runs inside the emit path. **W3 is DECIDED (2026-08-26) under "the code should not be spaghetti"**: one `--dressing {none,recognized,derived}` axis rather than a fourth boolean, `--dress-operators` kept as an alias so no existing invocation changes meaning, and the two emitters merged **after** W4/W5 as a deletion rather than before as an accumulation. The evidence is in the tree: `print_cpp_planck` has 16 branches and `dress_operators` already interacts at three points (`generate.py:1052/1064/1152`), and a comment at `:1060` records that "a second emit call site would fork the composition so V5 (UCC) had to be wired twice" — the same mistake, already paid for once. W3.3 (delete the old emitter) is the step that keeps it honest; if W3.3 will not happen, W3.1 should not start. **W4 is the one that can still find a correctness defect** — the value gate is symbolic and never compiles or runs a kernel; W2 compiles but does not execute. W4 is scoped in five steps, and its central trap is measured: **9 of the 10 CC regression cases never reach a generated kernel**, because `choose_determinant_backstop` (`tensor_backend.cpp:243`) routes `nso <= 16 && ndet <= 10000` to the determinant-space teaching backstop (`water_rccsdt_sto3g` even *asserts* that handoff in its `contains`). Only `ch4_rccsdt_sto3g` (nso=18, ndet=43758) exercises the tensor path — the same fact that left the hand-written tensor solver ungated for its entire life — so a W4 that runs "the CC cases" and reports green proves nothing for nine of ten. **W4.1 DONE (2026-08-26)**: baseline pinned at RHF `-39.7267328271` / CCSDT `-39.8058445095`, 0.18 s (note `--suite extended` is required; a bare `--case` selects nothing, silently). It also surfaced a **build blocker**: that case does not use generated kernels today and cannot in the current build — it asserts `kernels=hand-optimized`, logs `kernel=native` every iteration, and `PLANCK_RCCSDT_BACKEND=optimized` fails with "the generated rank-3 CCSDT kernel runs only in the arbitrary-order harness, which needs -DPLANCK_CC_ARBITRARY_LOWER_RANKS=ON" (confirmed OFF in `build/CMakeCache.txt`). So the comparison is **three-way**, not two: hand-written tensor (the baseline) / generated-undressed / generated-derivation-dressed. W4.2 is split accordingly — W4.2a reconfigures and checks generated-undressed against the baseline (needs nothing from W3, and separates a generated-kernel defect from a dressing defect), W4.2b adds W3.2's flag. W5 would be this route's first wall-clock number. **W4.2a and W4.5 LANDED (2026-08-26); W4's blocker is GONE.** W4.2a had recorded "the generated path does not converge; W4 is BLOCKED" — that was `PLANCK_CC_SPIN_ADAPT=OFF` (see `CCGEN_SPIN_ADAPT_DEFAULT`), not a kernel defect, and the flag now defaults ON. **The middle row of W4.1's table is established:** generated-undressed CCSDT on CH4 gives `-39.8058445098` against the hand-written baseline `-39.8058445095` — **3e-10**, inside the 1e-07 tolerance, with the generated path positively identified by its routing line rather than inferred. So a future W4.3 disagreement is attributable to dressing, not to the kernel or the harness — which was the point of splitting W4.2. **W4.5 also answered, by a different mechanism than it proposed:** `PLANCK_RCCSDT_BACKEND=tensor` does NOT bypass `choose_determinant_backstop` (it is called inside `run_tensor_rccsdt` off reference size alone; the env var picks among three backends), but **`optimized` does** — it routes through `rccgen.cpp` to the arbitrary-order harness, which never consults the backstop. So the `nso > 16 || ndet > 10000` limit recorded across several ccgen scopes binds the **hand-written** tensor path only. New case `lih_rccsdt_generated_sto3g` (nso=12, ndet=495, far below the threshold) exercises the generated route end to end, matches hand-written to all ten digits (`-7.8823242576`), PASSes in **5.3 s** against CH4's ~4 min, and is verified falsifiable. The generated route no longer rests on one case. **W3.1 LANDED (2026-08-26).** `--dressing {none,recognized,derived}` on `python/generate_planck_cc_kernels.py` — ONE dressing axis with a value, not a fourth boolean (`print_cpp_planck` already carries 16 branches and `dress_operators` interacts at three points; a comment at `generate.py:1060` records that a second emit call site already cost a double-wiring once). `--dress-operators` is kept as a deprecated alias resolved before anything reads it; `derived` parses but is refused with a message naming W3.2, so it cannot silently no-op; passing both spellings is an error, and the `--factorize-tau` mutual exclusion is preserved. `CMakeLists.txt:517` moved to the new spelling under the unchanged `PLANCK_CC_DRESS_OPERATORS` option. **Verified byte-identical** three ways: default vs `--dressing none`, alias vs `--dressing recognized`, and end to end through CMake (a fresh default tree's generated TUs are `cmp`-identical to the existing build's). Gated by `python/ccgen/tests/test_dressing_flag.py` (6 tests) — which also pins that `recognized` **changes** the output, so the enum cannot be accepted and then ignored. **W3.2 LANDED (2026-08-26): the derivation route has a PRODUCTION CALLER for the first time.** `dressing="derived"` is wired into `print_cpp_planck`. The blocker was a seam, not an algorithm: `emit_factorized_from_equations` called the emitter itself, so it could not feed the single downstream emit. Split out `factorize_equations(eqs,...) -> (rewritten_eqs, kept_specs)` — exactly the `(eqs, intermediates)` pair the recognition route already threads — with the old entry a thin delegate, **verified byte-identical before and after the split**. **The two routes run at different points, structurally:** `recognized` dresses BEFORE spin-adaptation (its hand-seeded specs declare GCC layouts that `adapt_intermediate_spec` transforms), `derived` factorizes AFTER (it derives operators FROM the adapted manifold, so its specs need no adaptation) — putting `derived` early would declare one layout and build another, the V1.2.2 defect class. **Verified:** production == the standalone bridge byte-identical (so production adds no behaviour the value gate never covered); default == `none`; legacy boolean == `recognized`; all three routes emit DISTINCT TUs; composes with `spin_adapt` (59 builders on spatial `ccsd`, matching W2); all three interaction points behave as for `recognized`. CLI refusal lifted. Gated by `test_dressing_derived.py` (7 tests); W3.1's refusal test **caught the change** and is rewritten as a positive assertion rather than deleted. `merge_transposes` deliberately NOT threaded (59 builders, not 31) — W3 says the merge is the end state and warns against absorbing the factorizer's seven knobs before W4/W5. The same **6 selection-model failures** (M2.0/M2.2/M2.3/M4/F3/E1) are confirmed pre-existing by running the suite on a stashed clean tree: 93 tests, 6 failures, before and after. **W4.2b/W4.3/W4.4 are now unblocked** (rebuild with `--dressing derived` and compare energies against W4.2a's `-39.8058445098`); then **W3.3 deletes the second emitter** — the step W3's own text calls the one that keeps this honest. **Not in scope:** UCC (the emitter rejects spin-blocked manifold names, `Unknown manifold 'singles_aa'`; needs O6 first), and the six selection-model gates |
