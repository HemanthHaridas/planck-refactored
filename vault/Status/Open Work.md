...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **U1.0/U1.1/U1.2/U1.5 landed, U1.3 DEAD, U1.4 blocked on a real defect.** Re-audited 2026-08-22 against the tree: U1.2's deliverable arrived via F3 (~6e-16 vs PySCF UCCSD), U1.5's is the F2.3 oracle (2.45e-15), and U1.3's hazard was designed out by U1.1 (its own gate passes with 0 violations across 2490 terms) rather than left open. **U1.4 then found a genuine rank-6 defect: with α ≡ β inputs the UCC manifold is NOT spin-flip symmetric — ~1e-1 relative at `ccsdt`, exact at `ccsd`** — pinned as `U14RankSixSpinFlipSymmetryTests` (rank-4 asserts, rank-6 `expectedFailure`). Not a block name, not the `v_abab` orientation, not the evaluator (all three ruled out inline). **This blocks U1.4 and any rank-6 numeric gate; U2+ C++ work should not start on rank 6 until it is fixed.** The oracle itself is ready — PySCF's `update_amps_uccsdt_tri_` plus the verified t3 block mapping are recorded in the scope doc |
