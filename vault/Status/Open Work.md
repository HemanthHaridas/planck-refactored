...
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` (U3/U4 rescope) | **U4 is ~90% done BY ACCIDENT, and that is the hazard.** `ucc_adapt_equations('ccsd')` → `emit_planck_translation_unit(...)` already emits a complete UCC TU today — six kernels, correct registry (`sector_tags` {1,"aa"} {1,"bb"} {2,"aaaa"} {2,"abab"} {2,"bbbb"}), no error. Amplitudes are genuinely block-resolved (U1.1 did its job). **The ERIs and Fock are not**: measured, `v_aaaa`/`v_abab`/`v_bbbb` ALL emit as `mo_blocks.oovv`, and `f_aa`/`f_bb` BOTH as `reference.f_ov`. `_map_eri_tensor` never receives the spin tag — `planck_tensor_cpp.py`'s integral branch matches `([vf])(?:_([ab]+))?` and uses only group(1). Its comment says the tag "does not change which space block it is"; right, but it changes which **array**, and under UHF <aa|aa>/<ab|ab>/<bb|bb> are three different integrals. Compiles, links, runs, returns a plausible energy — the B5 failure mode. Pinned by `test_ucc_emit_distinct_blocks.py` (**RED on purpose**, 3 pass / 2 fail), which is counting-based per factor, **not** a per-kernel comparison: a falsifiability probe showed both singles kernels legitimately reference *both* spin Fock blocks, so the obvious per-kernel assertion would have stayed red forever and been "fixed" by deletion. **So U3 is C++ AND emitter, one change** — doing `MOBlockCache` alone leaves the silent defect live and looks finished. **U3 is now scoped U3.0–U3.4, and scoping it turned up a SECOND emitter defect**: `_ERI_SYMMETRY_PERMUTATIONS` is applied spin-blindly, but 2 of its 4 perms (particle `<qp|sr>`, product `<sr|qp>`) map `abab` to `baba` — and **37 of 142** mixed-block reads use them today. Fixing only the array names sends those 37 to the right array with permuted indices, i.e. quieter than the current defect. Verified numerically on real orbitals: `baba == abab.transpose(1,0,3,2)`, and `abab` is NOT invariant under it. Two more measurements reshape the C++ half: `v_abab` needs **13** space patterns vs 7 for same-spin (the 8-fold symmetry cannot fold mixed spin), and the spin lives on the **chemists' charge-density pair**, not the physicist block — `oovv_abab` is `(i_α a_α | j_β b_β)`, a mixed transform, not `chem.ovov` of either pure spin, which is exactly where `rebind_physicist`'s oovv↔ovov cross-source bites. So the cache is NOT 3 copies of the RCC one. Do NOT store `baba` (recoverable by particle swap). U3.1's RHF-degenerate gate is **vacuous for the mixed pair ordering** (`C_α == C_β` makes `(a|b)` and `(b|a)` coincide) and needs an asymmetric companion. U4's real remainder is a `--ucc` CLI flag plus deciding whether an empty `residuals_by_rank` (measured: 0 pushes, every UCC target is tagged) needs a solver guard |
