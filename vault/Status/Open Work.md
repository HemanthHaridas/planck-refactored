...
| `CCGEN_WIRING_THE_DERIVATION_ROUTE` | **NEW (2026-08-25), not started.** The follow-through from `CCGEN_TWO_DRESSING_ROUTES`: the derivation route is gated and worth 2-7x but has **no production caller**. Scopes W1-W5. The gap is that ccgen has **two emitters** sharing exactly one parameter (`method`) — production calls `print_cpp_planck`, the factorizer's `emit_factorized_translation_unit` is called by nothing — and the factorizer generates its own equations internally, so it cannot emit spatial or UCC kernels. The machinery itself has no such limit (31 merged operators on spatial input, 86 on UCC), so W1 is a signature change. **W3 is a project-direction decision, not a technical one** (new flag / repoint `--dress-operators` / merge the emitters) and should not be taken unilaterally. **W4 is the one that can find a correctness defect** — the value gate is symbolic and never compiles or runs a kernel. **Not in scope:** UCC (needs O6 first), and the six selection-model gates |
