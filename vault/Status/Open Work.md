...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **Rank-4 UCC fully validated (~6e-16). Rank-6 singles/doubles exact (~1e-15); `triples_aaaaaa` open at ~1.2e-2 and scoped as U1.4c.** Four hypotheses are dead, each measured: T1-dressing convention (`t1=0` changes nothing), a spurious term family (the 18 suspect `t2·v` terms are textbook `P(i/jk)P(a/bc)` expansions), any integer over/under-count, and the t3 blocks themselves (singles/doubles consume t3 and are exact). **The blocker is the rank-6 closed-shell CLOSURE RELATIONS**: RCC reads one spatial `t3` while UCC's `triples_aabaab` needs all four blocks, so a wrong closure is indistinguishable from an equation defect — the same trap that produced the retracted 'rank-6 spin-flip defect'. Rank 4 has identical structure and passes only because its closure is known and simple. `t3_aaaaaa = (1/12)·𝒜_bra𝒜_ket t3_aabaab` reproduces both real pairs and is structurally sound on generic blocks, but is **not proven unique** — the images span rank 5 (vs 9 generic), the `bbb`←`bba` cross-check is passed by wrong fits too (shared null space), and refitting open-shell is **impossible** (α/β have different dimensions there; only 4 of 36 images are shape-legal). **Next: derive the closures from the spin integration rather than by fitting (U1.4c.1), then extend F2.3's oracle to rank 6 (U1.4c.2) — only then decide which side is wrong.** Do not resume the max-norm bisect: 38× cancellation plus an unproven closure cannot separate the causes. **Rank-6 triples untrusted; rank-4 safe** |
