...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **U1.0/U1.1/U1.2/U1.5 landed, U1.3 DEAD, U1.4 is the only open step.** Re-audited 2026-08-22 against the tree: U1.2's deliverable arrived via F3 (~6e-16 vs PySCF UCCSD), U1.5's is the F2.3 oracle (2.45e-15), and U1.3's hazard was designed out by U1.1 (its own gate passes with 0 violations across 2490 terms). U1.4's four candidate blockers all dissolved — oracle strength (PySCF ships `update_amps_uccsdt_tri_`), the t3 closure relation (already pinned green), oracle plumbing (solved), and a **suspected rank-6 equation defect that was actually a bug in the test fixture that found it** — `t3_abbabb` was set equal to `t3_aabaab` when a spin flip needs a slot reversal per half (`.transpose(2,1,0,5,4,3)`). The rank-6 manifold is spin-flip symmetric to **~7e-16**; `U14RankSixSpinFlipSymmetryTests` passes at both ranks and now also asserts each fixture block satisfies its own tag's antisymmetry — the check that would have caught it first. **Left: extend the fixture to t3, then run the direct rank-6 oracle.** No known correctness blocker to U2+ |
