...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **U1.4c.1–c.3 landed. ccgen's rank-6 T3 is CLEARED; the residual gap is at the PySCF interface.** The closed-shell oracle (`U14c2RankSixClosedShellOracleTests`, no PySCF) reproduces ccgen's own RCC at rank 6 on perturbed amplitudes — triples **1.5e-12** against ‖R‖~1e3. The PySCF gate (`test_ucc_rank6_vs_pyscf`, `expectedFailure`) still differs, and it is now characterized: an **additive offset independent of t3** (‖R‖ grows 250× while the difference stays ~1e-2), scaling with t1/t2 instead, and **present at PySCF's exactly converged amplitudes with nothing perturbed — 4.3e-3 against ‖R‖ 4.9e-2, 8.8% relative, while singles/doubles agree to ~1e-15 on the same run**. That unperturbed case is the cleanest thing to debug against. The residuals are nearly parallel (cos 0.99999, scale 0.99925) with no single term explaining the gap (max per-term cosine 0.22) — a uniform deficit, not a missing term. Ruled out by measurement: the denominator (`D3` matches PySCF's `eijkabc` to 2.8e-14; `focka.diagonal() == mo_energy`; `level_shift=0`), the packed round-trip (≤3.5e-17, both sides see the same t3), the closure relations, and antisymmetry. **Rank-4 UCC fully validated; rank-6 triples untrusted against PySCF but no longer suspected of being a ccgen defect — do not block U2+ on it** |
