...
| `CCGEN_WIRING_THE_DERIVATION_ROUTE` | **W1-W2 LANDED (2026-08-26); W3-W5 open.** The follow-through from `CCGEN_TWO_DRESSING_ROUTES`: the derivation route is gated and worth 2-7x but has **no production caller**. **The technical blocker is gone** — `emit_factorized_from_equations` (W1) takes the equations instead of generating them, byte-identical over six configurations, and the spatial TU now **compiles** against the real CC headers merged and un-merged, 59 → 31 builders (W2). The emitter's ERI set is re-verified as exactly the four parity-`+1` relations, which matters now that merging runs inside the emit path. **W3 is DECIDED (2026-08-26) under "the code should not be spaghetti"**: one `--dressing {none,recognized,derived}` axis rather than a fourth boolean, `--dress-operators` kept as an alias so no existing invocation changes meaning, and the two emitters merged **after** W4/W5 as a deletion rather than before as an accumulation. The evidence is in the tree: `print_cpp_planck` has 16 branches and `dress_operators` already interacts at three points (`generate.py:1052/1064/1152`), and a comment at `:1060` records that "a second emit call site would fork the composition so V5 (UCC) had to be wired twice" — the same mistake, already paid for once. W3.3 (delete the old emitter) is the step that keeps it honest; if W3.3 will not happen, W3.1 should not start. **W4 is the one that can still find a correctness defect** — the value gate is symbolic and never compiles or runs a kernel; W2 compiles but does not execute. W4 is scoped in five steps, and its central trap is measured: **9 of the 10 CC regression cases never reach a generated kernel**, because `choose_determinant_backstop` (`tensor_backend.cpp:243`) routes `nso <= 16 && ndet <= 10000` to the determinant-space teaching backstop (`water_rccsdt_sto3g` even *asserts* that handoff in its `contains`). Only `ch4_rccsdt_sto3g` (nso=18, ndet=43758) exercises the tensor path — the same fact that left the hand-written tensor solver ungated for its entire life — so a W4 that runs "the CC cases" and reports green proves nothing for nine of ten. **W4.1 DONE (2026-08-26)**: baseline pinned at RHF `-39.7267328271` / CCSDT `-39.8058445095`, 0.18 s (note `--suite extended` is required; a bare `--case` selects nothing, silently). It also surfaced a **build blocker**: that case does not use generated kernels today and cannot in the current build — it asserts `kernels=hand-optimized`, logs `kernel=native` every iteration, and `PLANCK_RCCSDT_BACKEND=optimized` fails with "the generated rank-3 CCSDT kernel runs only in the arbitrary-order harness, which needs -DPLANCK_CC_ARBITRARY_LOWER_RANKS=ON" (confirmed OFF in `build/CMakeCache.txt`). So the comparison is **three-way**, not two: hand-written tensor (the baseline) / generated-undressed / generated-derivation-dressed. W4.2 is split accordingly — W4.2a reconfigures and checks generated-undressed against the baseline (needs nothing from W3, and separates a generated-kernel defect from a dressing defect), W4.2b adds W3.2's flag. W5 would be this route's first wall-clock number. **Not in scope:** UCC (the emitter rejects spin-blocked manifold names, `Unknown manifold 'singles_aa'`; needs O6 first), and the six selection-model gates |
| `CCGEN_RANK3_ARBITRARY_KERNEL_DEFECT` | **NEW (2026-08-26), BLOCKING W4.** The generated rank-3 CCSDT kernel does not converge: `ch4_rccsdt_sto3g` through the arbitrary-order harness gives `E_corr = -0.0565650696` against the hand-written `-0.0791116825`, hitting the 100-iteration cap. Not DIIS (both settings reach the same value), not the cap (`E_corr` flat from iter 45 while only the residual decays), and **not a regression** — commit `1986d0c`, whose message claims "+1.44e-08, agrees with the hand-written path to 1.0e-10", reproduces the failure **bit-identically** when checked out and rebuilt. **The claim was never reproducible**, and `Completion.md` is corrected. It survived because the gate that commit added asserts `kernels=hand-optimized` — the hand-written path — so it has been green while never running the generated kernel it was added to protect, the same coverage hole that commit's own message diagnoses one level up. **Localized:** rank 2 (`-39.8056549601`) and rank 4 in the default build (`-14.4036550465`, 13 iters) both work through the same harness/solver/DIIS/denominators, so the defect is the rank-3 kernel. **Second consequence:** with `ARBITRARY_LOWER_RANKS=ON` rank 4 warm-starts from the broken rank-3 solve and fails too (R2 fixes that separately). Ladder R1-R4. **R1 LANDED (2026-08-26)**: `ch4_rccsdt_generated_sto3g` forces the generated path (per-case `env` added to the runner, since it is reachable only via `PLANCK_RCCSDT_BACKEND`) and pins the OBSERVED behaviour — exit 1, the non-convergence message, and `E_corr=-0.0565650696` — rather than the correct energy, so it discriminates "converged wrong" from "did not converge". **When the defect is fixed this case must FAIL**; the JSON carries a `known_failure` field saying exactly how to invert it. Skips cleanly in a default build via `requires_build_option`. **R3 ANSWERED (2026-08-26): the kernel is WRONG, not cold-started into a bad basin.** Its scoped experiment was unimplementable (RCC has no rank-2 generated keyword, deliberately), but a better discriminator was already in the tree: the Be CCSDTQ run performs a rank-3 sub-solve that **converges** — 12 iterations, `rms(res)=1.9e-12` — to `E_corr = -0.0139349127` against the hand-written `-0.0517702884`, a **3.78e-02** gap. A tight residual at a wrong energy means the kernel is driving a DIFFERENT equation to self-consistency. That also refutes `1986d0c`'s "BITWISE IDENTICAL across both harnesses" claim independently of the CH4 reproduction, and makes R2 more urgent: rank 4 is seeded from amplitudes that are converged and wrong, which nothing downstream can detect. **R4 scoped in five steps** on the Be fixture (12 iters / 0.01 s, versus CH4's 100 / 3.4 s and no convergence), narrowing equations -> emission -> block binding -> packing; the solver is already exonerated by ranks 2 and 4 working through it. **R4.1 DONE (2026-08-26): the equations are clean, the defect is in the C++ layer.** PySCF lives in `tests/pyscf/.venv` (2.13.0), not any conda env — the earlier skip was an environment artifact, not evidence. Run there, `test_ccgen_ccsdt_reaches_fci_limit` **passes** (generated CCSDT residual reaches FCI to 8 places), and the reference suite goes 20-skipped -> **31/32 passing**. The suite also hands over the decisive comparison on **Be/STO-3G, R3's exact system**: generated equations solved in **Python** give `E_corr = -0.0517702744`, hand-written C++ gives `-0.0517702884` (**agree to 1.40e-08**), and the generated C++ kernel gives `-0.0139349127` (**off by 3.78e-02**). Same equations, right answer in Python, wrong answer through the C++ kernel — so the defect is emission / block binding / packing, and `1986d0c`'s "BITWISE IDENTICAL across both harnesses" is settled false. R4.2 next: diff the residual element-wise at R4.1's converged amplitudes, where the correct residual is known to be ~0 |
