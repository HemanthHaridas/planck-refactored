...
| `CCGEN_UCC_RESIDUAL_EVALUATOR_SCOPE` | **F2.0/F2.0b/F2.1/F2.2a landed** — `v`/`f` block-tagged, emitter accepts the tag, `ucc_resolve_factor`, `ucc_term_spins`. **F2.2b–F2.4 open**, and F2.3 (the closed-shell oracle) is the load-bearing one. Until it runs, every landed UCC residual is gated **structurally only** — distinct names, non-empty blocks, symmetric counts — and nothing has checked their *values* |
