...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **U1.4c.1 + U1.4c.2 LANDED — ccgen's rank-6 T3 is CLEARED.** The closed-shell oracle (`U14c2RankSixClosedShellOracleTests`, no PySCF needed) reproduces ccgen's own RCC residual on *perturbed* amplitudes at rank 6: singles 2.3e-13, doubles 1.1e-12, **triples 1.5e-12** against ‖R‖~1e2–1e3. So the equations are self-consistent, and the remaining ~1.1e-2 disagreement with PySCF (`test_ucc_rank6_vs_pyscf`, `expectedFailure`) is **in how that gate reads PySCF's `r3aaa`, not in ccgen** — U1.4c.3 is now answerable rather than guessable. Both closures are derived and asserted load-bearing (corrupting them breaks the oracle by 89.0 / 51.3): `t3_aaaaaa` from `_split_same_spin_amplitude` read as the **inverse** axis permutation, `t3_abbabb = t3_aabaab.transpose(2,0,1,5,3,4)`. Measured for U1.4c.3 and deliberately not acted on: deriving the gate's `aaa`/`bbb` from the perturbed mixed block makes the closure hold there but leaves the discrepancy at 1.1e-2, so that is **not** the cause. Rank-4 UCC fully validated; **rank-6 triples still untrusted against PySCF, though no longer suspected of being a ccgen defect** |
