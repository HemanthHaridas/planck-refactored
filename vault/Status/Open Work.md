...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **U1.0–U1.3 and U1.5 settled; U1.4's gate runs with ONE localized discrepancy.** Rank-4 UCC is fully PySCF-validated (~6e-16). The rank-6 gate (`U14RankSixVsPyscfTests`, water/6-31g) reproduces PySCF UCCSDT for **singles ~5e-16 and doubles ~1.4e-15**; **`triples_aaaaaa` is off by ~1.2e-2 against ‖R‖~3.1 and is `expectedFailure`.** Localized to **18 terms of shape `t2_aaaa · v_aaaa`** — the T2→T3 generating term: at `t1=t3=0` ccgen gives 1.27e-2 where PySCF gives 3.3e-3 (ccgen's t2-only contribution is ~4× the entire reference), every other ccgen term evaluates to exactly 0 there, and dropping those 18 leaves 4.9e-3 vs 3.3e-3. **Which side is wrong is NOT established** — a T1-dressing convention difference was the leading hypothesis and is dead (`t1=0` leaves the discrepancy unchanged); resolve it against a third source, not by re-reading either. Three gate defects were fixed to get here and are worth knowing: **N2/STO-3G's converged `t2ab` is not reproducible across processes** (gauge freedom the energy does not see), **water/STO-3G has `nv=2` so same-spin triples are identically zero** (vacuous pass), and **`f_ov` must be zeroed on the PySCF side too** — omitting it set a 7e-10 floor that tightening `conv_tol` made worse. **Rank-6 triples should not be trusted until this closes; rank-4 is safe** |
