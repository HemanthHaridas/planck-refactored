...
| scope | state |
|---|---|
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` + `CCGEN_U1_UCC_ADAPT_SCOPE` | **U1.0–U1.3 and U1.5 settled; U1.4's gate runs, with ONE open discrepancy.** U1.2 landed via F3 (~6e-16 vs PySCF UCCSD), U1.5 is the F2.3 oracle (2.45e-15), U1.3 is DEAD (U1.1 designed its hazard out). U1.4's rank-6 gate (`U14RankSixVsPyscfTests`) now reproduces PySCF UCCSDT for **singles ~5e-14 and doubles ~3e-15**, but **`triples_aaaaaa` is off by ~1.4e-2 and is `expectedFailure` — the one genuinely open UCC item.** The split localizes it: rank-6 singles/doubles *consume* t3 and are exact, so the t3 blocks and ccgen's reading of them are correct and the defect is in the **T3 equation** (219/579 terms carry t3). Ruled out: layout or symmetry (both residuals bra-antisym to 4e-16, and their difference too), a scale factor (ratio median 0.9969, additive not multiplicative), the fixture blocks, the packing round-trip. Note also an earlier scare that was NOT real: a suspected rank-6 spin-flip asymmetry was a bug in the test fixture (`t3_abbabb` needs `.transpose(2,1,0,5,4,3)`, not equality with `t3_aabaab`) — the manifold is symmetric to ~7e-16. **Rank-4 UCC is fully validated; rank-6 triples should not be trusted until this closes** |
