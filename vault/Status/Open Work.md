...
| `CCGEN_ARBITRARY_ORDER_UCC_SCOPE` (U3/U4) | **U3 LANDED, U3.0–U3.4.** Spin-blocked ERIs and Fock, emitter routing, open-shell MP2 limit. The emitted UCC TU now has **zero** untagged reads of either kind (`mo_blocks.<space>(` or `reference.f_<space>(`), and the U2.0 pre-gate is green. RCC emit byte-identical throughout (SHA-256 unchanged on ccsd/ccsdt at every step). **Two things the scope did not predict.** (1) The per-tag canonical block set had to be **derived**, not shared: `_CANONICAL_ERI_BLOCKS`'s 8-fold orbit reaches all 16 o/v patterns, but a mixed block's orbits reach only 11, so the first restricted emit raised `NotImplementedError` on `vovv` — 6 stored arrays same-spin, **10** mixed, matching U3.1's cache. (2) U3.4 needed **no solver**: scoped as a Jacobi step (which needs U5), but first-order MP2 amplitudes are closed-form, so the energy assembles straight from U3.1's integrals + U2.1's denominators against an independent transform — stronger (fails if either half is wrong OR they are misaligned) and 0.01 s. **Two gate defects found by mutation, both of which would have certified a broken fix**: the U3.0 assertion re-implemented the routing inline instead of calling the emitter, so it measured a *simulation* of the old code and stayed red at 37 after the fix landed (**a gate that cannot observe the fix cannot certify it**); and a spin-blind-permutation mutation SURVIVED the rewritten gate, because it still binds only legitimate blocks — what moves is which array each *read* names and in what *index order*, invisible to anything inspecting the bound set. **Remaining: U4** (a `--ucc` CLI flag; `ucc_adapt_equations` still has zero non-test callers; plus deciding whether an empty `residuals_by_rank` — measured 0 pushes, every UCC target is tagged — needs a solver guard) **and U5** (driver routing + the UCCSDTQ==FCI gate) |
