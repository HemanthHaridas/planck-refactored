...
namespace
{
        ...
        for (std::size_t mu = 0; mu < nb; ++mu)
        {
            ...
            for (std::size_t nu = 0; nu < nb; ++nu)
            {
                ...
                if (cv_nu._shell != tgt_shell)
                    continue; // must come from the corresponding shell
        ...
        std::vector<std::string> &labels)
    {
        ...
        for (int i = 0; i < nb; ++i)
        {
            ...
            {
                const double d = (chi_pi2 - m.c1) * (chi_pi2 - m.c1) + (chi_pi3 - m.c2) * (chi_pi3 - m.c2);
                if (d < best_dist)
                {
                    best_dist = d;
                    best_lam = m.lam;
            ...
            if (best_lam == 0)
            {
                ...
                label = "LAM" + std::to_string(best_lam);
                                    ...
                                    std::vector<std::string> &labels)
    {
        ...
        if (b1_idx < 0 || b2_idx < 0)
            return; // group has no separate B1/B2
...
HartreeFock::Symmetry::SAOBasis HartreeFock::Symmetry::build_sao_basis(HartreeFock::Calculator &calculator)
{
    ...
    // unchanged — this is intentional since the SAOs themselves are correct.
    {
        ...
        for (int g = 0; g < n_irreps; ++g)
        {
            if (result.irrep_names[g] == "B1")
                b1_idx = g;
            if (result.irrep_names[g] == "B2")
                b2_idx = g;
        }
        if (b1_idx >= 0 && b2_idx >= 0)
        {
