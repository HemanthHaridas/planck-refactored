                          ...
                          Vec3& sA, Vec3& sB)
{
    ...
    double R = rAB.norm();
    if (R < 1e-10) { sA.setZero(); sB.setZero(); return; }
    sA =  rAB / R;
    sB = -sA;
}
...
static double bend_value(const Eigen::MatrixXd& xyz, int A, int B, int C)
{
    ...
    if (d1 < 1e-10 || d2 < 1e-10) return 0.0;
                       ...
                       Vec3& sA, Vec3& sB, Vec3& sC)
{
    ...
    if (dBA < 1e-10 || dBC < 1e-10) {
        sA.setZero(); sB.setZero(); sC.setZero(); return;
    }
    Vec3 e1 = rBA / dBA;
    Vec3 e2 = rBC / dBC;
    ...
    if (sin_t < 1e-8) { sA.setZero(); sB.setZero(); sC.setZero(); return; }
                              ...
                              int A, int B, int C, int D)
{
    ...
    if (mm < 1e-20 || nn < 1e-20) return 0.0;
    double b2n = b2.norm();
    double x   = m.dot(n);
                          ...
                          Vec3& sA, Vec3& sB, Vec3& sC, Vec3& sD)
{
    ...
    if (mm < 1e-20 || nn < 1e-20) {
        sA.setZero(); sB.setZero(); sC.setZero(); sD.setZero(); return;
    }

    sA = -b2n / mm * m;
    sD =  b2n / nn * n;
    sB = (b1.dot(b2) / b2n2 - 1.0) * (b2n / mm) * m
       + (b3.dot(b2) / b2n2)        * (b2n / nn) * n;
    sC = -(sA + sB + sD);
}
    ...
    const Eigen::MatrixXd& xyz_bohr, const Eigen::VectorXi& Z)
{
    ...
    for (int B = 0; B < N; ++B) {
        ...
        for (int ia = 0; ia < static_cast<int>(nb.size()); ++ia) {
            for (int ic = ia + 1; ic < static_cast<int>(nb.size()); ++ic) {
                ...
                if (ang < ang_lo || ang > ang_hi) continue;
    ...
    for (const auto& [B, C] : bond_list) {
        for (int A : adj[B]) {
            ...
            if (ang1 < ang_lo || ang1 > ang_hi) continue;

            for (int D : adj[C]) {
                if (D == B || D == A) continue;
                double ang2 = bend_value(xyz_bohr, B, C, D);
                if (ang2 < ang_lo || ang2 > ang_hi) continue;
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        for (int i = 0; i < nics(); ++i) {
            ...
            if (coords[i].type == ICType::Torsion) {
                while (dq_actual[i] >  M_PI) dq_actual[i] -= 2.0 * M_PI;
                while (dq_actual[i] < -M_PI) dq_actual[i] += 2.0 * M_PI;
            }
        }
        Eigen::VectorXd residual = dq - dq_actual;
        if (residual.norm() < 1e-10) break;
