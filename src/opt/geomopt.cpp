    ...
    const GradientRunner &gradient_runner)
{
    ...
    for (const auto &con : calc._constraints)
    {
        ...
        {
            ic.type = ICType::Torsion;
            ic.atoms = {a0, a1, a2, a3};
        }
    ...
    if (!calc._constraints.empty())
    {
        ...
        for (bool f : ic_frozen)
            if (f)
                ++n_frozen_ic;
        for (bool f : atom_frozen)
            if (f)
                ++n_frozen_atom;
    ...
    for (int iter = 0; iter < max_iter; ++iter)
    {
        ...
        if (sy > 1e-10)
        {
            ...
            H += (y_bfgs * y_bfgs.transpose()) / sy - (Hs * Hs.transpose()) / sHs;
        }
