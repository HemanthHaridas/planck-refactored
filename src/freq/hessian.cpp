...
static Eigen::MatrixXd _eckart_basis(const HartreeFock::Calculator& calc)
{
    ...
    for (std::size_t a = 0; a < N; ++a) {
        m[a]  = element_from_z(static_cast<uint64_t>(
                    calc._molecule.atomic_numbers[a])).mass;
        sq[a] = std::sqrt(m[a]);
    }
    ...
    for (std::size_t a = 0; a < N; ++a) {
        ...
        //   e_y × d = (-dz,  0,  dx)
        //   e_z × d = ( dy,-dx,   0)
        D(i + 0, 3) =  0.0;   D(i + 1, 3) = -s * d[2]; D(i + 2, 3) =  s * d[1];
        D(i + 0, 4) =  s * d[2]; D(i + 1, 4) =  0.0;   D(i + 2, 4) = -s * d[0];
        D(i + 0, 5) = -s * d[1]; D(i + 1, 5) =  s * d[0]; D(i + 2, 5) =  0.0;
    }
