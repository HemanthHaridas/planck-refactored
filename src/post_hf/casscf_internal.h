...
namespace HartreeFock::Correlation::CASSCFInternal
{
    ...
    inline CIString low_bit_mask(int nbits)
    {
        if (nbits <= 0)
            return 0;
        if (nbits >= kCIStringBits)
            return std::numeric_limits<CIString>::max();
    ...
    inline bool admissible_ras_pair(CIString alpha, CIString beta, const RASParams &ras)
    {
        if (!ras.active)
            return true;
        return ras1_holes(alpha, beta, ras) <= ras.max_holes && ras3_electrons(alpha, beta, ras) <= ras.max_elec;
    }
        ...
        const SymmetryContext &sym_ctx)
    {
        ...
        for (int t = 0; t < static_cast<int>(irr_act.size()); ++t)
        {
            if (irr_act[t] < 0)
                continue;
            if (alpha & single_bit_mask(t))
                sym = sym_ctx.product[sym][irr_act[t]];
            if (beta & single_bit_mask(t))
                sym = sym_ctx.product[sym][irr_act[t]];
        }
        return sym;
    }
        ...
        double precond_floor = 1e-4)
    {
        ...
        if (H.rows() != H.cols() || H.rows() != c0.size() || H.rows() != H_diag.size() || H.rows() != sigma.size())
            return result;
