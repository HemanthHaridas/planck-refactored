...
namespace HartreeFock::Correlation::CASSCF
{
    ...
    namespace
    {
        // RI is active for these builders only when a Calculator is threaded in
        // and it has density fitting enabled.
        inline bool ri_active(const HartreeFock::Calculator *ri_calc)
        {
            return ri_calc != nullptr && ri_calc->_mp2.use_ri;
        }
    }
    ...
    ActiveIntegralCache build_active_integral_cache(
        ...
        int n_act,
        int nbasis,
        HartreeFock::Calculator *ri_calc)
    {
        ...
        if (ri_active(ri_calc))
        {
            auto puvw = HartreeFock::Correlation::transform_eri_active_cache_ri(
                *ri_calc, C, C_act);
            if (!puvw)
                return cache; // invalid; caller sees valid == false
            cache.puvw = std::move(*puvw);
        }
        else
        {
    ...
    Eigen::MatrixXd build_inactive_fock_mo(
        ...
        int n_core,
        int nbasis,
        const HartreeFock::Calculator *ri_calc)
    {
        ...
        const Eigen::MatrixXd G =
            ri_active(ri_calc)
                ? HartreeFock::Correlation::RI::build_ri_fock_rhf(*ri_calc, D)
                : HartreeFock::ObaraSaika::_compute_fock_rhf(eri, D, nbasis);
        ...
        const HartreeFock::Calculator *ri_calc)
    {
        ...
        const Eigen::MatrixXd G =
            ri_active(ri_calc)
                ? HartreeFock::Correlation::RI::build_ri_fock_rhf(*ri_calc, D_act)
                : HartreeFock::ObaraSaika::_compute_fock_rhf(eri, D_act, nbasis);
...
#pragma omp parallel for schedule(static)
        ...
        HartreeFock::Calculator *ri_calc)
    {
        ...
        const Eigen::MatrixXd F_I_mo =
            build_inactive_fock_mo(C, H_core, eri, n_core, nbasis, ri_calc);
        const Eigen::MatrixXd F_A_mo =
            build_active_fock_mo(C, gamma, eri, n_core, n_act, nbasis, ri_calc);
        const ActiveIntegralCache active_integrals =
            build_active_integral_cache(eri, C, n_core, n_act, nbasis, ri_calc);
