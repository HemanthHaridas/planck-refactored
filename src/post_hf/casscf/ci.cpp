...
namespace HartreeFock::Correlation::CASSCF
{
    ...
    namespace
    {
            ...
            int target_irr)
        {
            ...
            for (int ia = 0; ia < na; ++ia)
                for (int ib = 0; ib < nb; ++ib)
                    if (CASSCFInternal::admissible_ras_pair(a_strs[ia], b_strs[ib], ras) && (!do_sym || CASSCFInternal::determinant_symmetry(a_strs[ia], b_strs[ib], irr_act, *sym_ctx) == target_irr))
            ...
            int max_iter)
        {
            ...
            std::sort(order.data(), order.data() + dim, [&](int a, int b)
        ...
        int n_act)
    {
        ...
        if (n_diff == 0)
        {
            ...
            {
                if (ket_a & CASSCFInternal::single_bit_mask(k))
                    val += h_eff(k, k);
                if (ket_b & CASSCFInternal::single_bit_mask(k))
                    val += h_eff(k, k);
            }
        ...
        if (n_diff_a == 1 && n_diff_b == 0)
        {
            ...
            {
                if (!(ket_a & CASSCFInternal::single_bit_mask(r)) || r == p)
                    continue;
                val += sgn * (g_act(ga, p, q, r, r, n_act) - g_act(ga, p, r, r, q, n_act));
            }
            for (int r = 0; r < n_act; ++r)
            {
                if (!(ket_b & CASSCFInternal::single_bit_mask(r)))
                    continue;
        ...
        if (n_diff_a == 0 && n_diff_b == 1)
        {
            ...
            for (int r = 0; r < n_act; ++r)
            {
                if (!(ket_b & CASSCFInternal::single_bit_mask(r)) || r == p)
                    continue;
                val += sgn * (g_act(ga, p, q, r, r, n_act) - g_act(ga, p, r, r, q, n_act));
            }
            for (int r = 0; r < n_act; ++r)
            {
                if (!(ket_a & CASSCFInternal::single_bit_mask(r)))
                    continue;
                val += sgn * g_act(ga, p, q, r, r, n_act);
            }
            return val;
