...
std::vector<double> pack_sigma(const DensityOnGrid& density)
{
    ...
    const Eigen::VectorXd sigma_ab =
        (density.alpha.grad_x.array() * density.beta.grad_x.array()
       + density.alpha.grad_y.array() * density.beta.grad_y.array()
       + density.alpha.grad_z.array() * density.beta.grad_z.array()).matrix();
...
{
    return (grad_x.array().square()
          + grad_y.array().square()
          + grad_z.array().square()).matrix();
}
