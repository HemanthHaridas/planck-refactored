    ...
    const std::string& label)
{
    ...
    std::stable_sort(order.begin(), order.end(),
        [&](int left, int right) { return energies_sao[left] < energies_sao[right]; });
    ...
    for (unsigned int iter = 1; iter <= max_iter; ++iter)
    {
        ...
        if (use_diis)
        {
            const Eigen::MatrixXd error_alpha =
                X.transpose() *
                (fock_alpha * alpha_density * calculator._overlap
                 - calculator._overlap * alpha_density * fock_alpha) * X;
            const Eigen::MatrixXd error_beta =
                X.transpose() *
                (fock_beta * beta_density * calculator._overlap
                 - calculator._overlap * beta_density * fock_beta) * X;
