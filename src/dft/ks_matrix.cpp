...
{
    return (coefficient.x() * ao_grid.grad_x.row(point).transpose()
          + coefficient.y() * ao_grid.grad_y.row(point).transpose()
          + coefficient.z() * ao_grid.grad_z.row(point).transpose()).eval();
}
    ...
    for (Eigen::Index point = 0; point < npoints; ++point)
    {
        ...
        if (xc_grid.vsigma.cols() == 3)
        {
            ...
            const Eigen::Vector3d coefficient_alpha =
                2.0 * xc_grid.vsigma(point, 0) * grad_alpha
              +       xc_grid.vsigma(point, 1) * grad_beta;
            const Eigen::Vector3d coefficient_beta =
                      xc_grid.vsigma(point, 1) * grad_alpha
              + 2.0 * xc_grid.vsigma(point, 2) * grad_beta;
