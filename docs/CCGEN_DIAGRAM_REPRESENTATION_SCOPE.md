...
- **AR3.2 — FCI limit (~M, remaining).** Iterate the generated CCSDT residual to
  convergence on a 2-electron system (H₂/STO-3G) and compare to FCI
  (`src/post_hf/fci.cpp`): CCSDT on 2 e⁻ = FCI exactly. Needs a working amplitude
  iterate (the AR3.3 note's layout wrinkle must be resolved for a self-consistent
  solve, or use the C++ `solver_arbitrary.cpp`). Convention-independent.
  *(historical note: the paragraph below described AR3.3 as the decisive test
  before it landed; retained for the convention argument.)*
  A wrong residual converges to the wrong fixed point
  or not at all, so matching energy validates the equations end-to-end — and the
  *energy is convention-independent*, so ccgen's bare projection and PySCF's
  T1-dressed `rccsdt` must agree despite unlike per-term residuals.

Together AR3.1 (reduction) + AR3.2 (FCI limit) + AR3.3 (energy) validate the
CCSDT weights to the standard production codes actually meet — **without a
per-term CCSDT oracle**. This replaces the old B0 residual-tensor build.

**The weight extension itself (what AR3.1–3.3 then gate):**
- **Magnitude:** T3/T4 amplitudes carry `(1/n!)²` (1/36, 1/576), so weights stop
  being dyadic (measured: `diagram_magnitude` currently produces only dyadic
  values on the 47 triples diagrams — the T3 factor is missing). Extend
  `diagram_magnitude` with the amplitude-normalization factor.
- **Sign:** `structural_sign` now uses the rank-general `external_pairing_parity`
  (B1, landed) so it is correctly *shaped* at triples; correctness is gated by
  AR3.1–3.3 (not a per-diagram sign oracle, which doesn't exist past CCSD).
- **AR1 (LANDED):** all 74 ccsdtq diagrams assemble, each `P(ijkl)P(abcd)` orbit
  residual is antisymmetric and the 74 are linearly independent (rank 74/74, at
  `no=nv=6`). Well-formedness only; correctness rides AR3.1/AR3.3 up the ranks.
