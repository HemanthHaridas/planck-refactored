#!/usr/bin/env python3
"""Build a static HTML page from the Planck architecture guide markdown.

The source file (ARCHITECTURE.md) contains <div align="justify"> wrappers
that are used by Markdown previewers which do not support CSS text-align.
Those tags are stripped before rendering so the HTML stylesheet can handle
justification cleanly via `text-align: justify`.

Math notation (if any):
  Display math  -- delimited by \\[ ... \\]
  Inline math   -- delimited by \\( ... \\)

MathJax 3 is loaded from the jsDelivr CDN and renders all LaTeX at page
load. Math regions are extracted before any HTML escaping so that
backslashes, angle brackets, and other LaTeX syntax are preserved verbatim.
"""
...
# ── div-tag stripping ─────────────────────────────────────────────────────
#
# ARCHITECTURE.md wraps prose paragraphs in <div align="justify">...</div>
# for Markdown preview compatibility.  The HTML stylesheet handles
# justification via CSS, so these tags must be removed before rendering.

def _strip_div_tags(text: str) -> str:
    """Remove <div align="justify"> and </div> lines from the markdown."""
    lines = text.splitlines(keepends=True)
    out: list[str] = []
    for line in lines:
        stripped = line.strip()
        if stripped == '<div align="justify">' or stripped == "</div>":
            continue
        out.append(line)
    return "".join(out)
...
def _restore_math(html_text: str, store: dict[str, str]) -> str:
    ...
    for tok, latex in store.items():
        if latex.startswith(r"\["):
            replacement = f'<span class="math-display">{latex}</span>'
        else:
            replacement = f'<span class="math-inline">{latex}</span>'
...
def render_markdown(markdown_text: str) -> tuple[str, list[TocEntry]]:
    ...
    for raw_line in lines:
        ...
        if heading:
            ...
            output.append(f'<h{level} id="{anchor}">{tag}</h{level}>')
            continue
        ...
        if (in_list or in_ordered_list) and re.match(r"^ {2,}", line):
            ...
            if output and output[-1].endswith("</li>"):
                inner = output[-1][4:-5]
                output[-1] = (
                    f"<li>{inner} {inline_format(line.strip())}</li>"
                )
            continue
            ...
            if all(re.fullmatch(r"[-: ]+", c) for c in cells):
                if pending_header is not None:
                    ...
                    for c in pending_header:
                        output.append(f"<th>{inline_format(c)}</th>")
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .toc-level-3 a {{ padding-left: 24px; font-size: 0.85rem; }}
    .toc-level-4 a {{ padding-left: 36px; font-size: 0.82rem; }}
    ...
    .content th {{
      ...
      padding: 10px 14px;
      text-align: left;
      background: rgba(154,61,34,0.08);
      font-weight: 600;
      font-size: 0.93rem;
      white-space: nowrap;
    }}
    ...
    @media (max-width: 600px) {{
      ...
      .toc-level-4 a {{ padding-left: 38px; font-size: 0.87rem; }}
    }}
  </style>
</head>
<body>
  <div class="page">
    <section class="hero">
      <h1>{et}</h1>
      <p>A detailed reference of every module in the Planck codebase —
         design decisions, data-flow, trade-offs, and key invariants for
         RHF/UHF SCF, MP2, CASSCF/RASSCF, analytic gradients, geometry
         optimisation, vibrational analysis, and KS-DFT.</p>
    </section>
    <div class="layout">
      <aside class="sidebar">
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a static HTML page from the architecture guide markdown"
    )
    parser.add_argument("--input",  default="docs/ARCHITECTURE.md")
    parser.add_argument("--output", default="docs/architecture.html")
    ...
    # 1. Strip <div align="justify"> / </div> tags added for MD preview
    markdown_text = _strip_div_tags(markdown_text)

    # 2. Extract math regions → tokens
    markdown_no_math, math_store = _extract_math(markdown_text)

    # 3. Render markdown to HTML
    content_html, toc = render_markdown(markdown_no_math)

    # 4. Restore math tokens as MathJax-ready LaTeX
    content_html = _restore_math(content_html, math_store)

    # 5. Build TOC and assemble page
    toc_html = build_toc(toc)
    title = "Planck — Architecture Reference"
