...
def run_planck_matrices(executable: Path, input_path: Path) -> dict[str, np.ndarray]:
    ...
    if not mats:
        raise RuntimeError("No PLANCK_RMP2_MATRIX rows were found in Planck output.")
    return mats


def build_pyscf_chain(input_path: Path) -> dict[str, np.ndarray]:
    ...
    if with_frozen:
        raise RuntimeError("Frozen-orbital chain comparison is not implemented in this script.")
    ...
    p_ao = reduce(np.dot, (mo_coeff, p_mo, mo_coeff.T))

    dm1_corr_relaxed_ao = reduce(np.dot, (mo_coeff, corr_relaxed_mo, mo_coeff.T))
    dm1p = hf_dm1 + dm1_corr_relaxed_ao * 2.0
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare Planck and PySCF restricted response-density matrices.")
    ...
    planck = run_planck_matrices(executable, input_path)
    pyscf = build_pyscf_chain(input_path)

    order = ["z", "corr_relaxed_mo", "P_ao", "dm1_corr_relaxed_ao", "dm1p"]
    print("Restricted response-density chain: Planck vs PySCF")
    for name in order:
        summarize_diff(name, planck[name], pyscf[name])
    return 0
