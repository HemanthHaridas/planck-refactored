...
H     -0.758602     0.000000     0.504284
"""
    mol.basis = helper._load_basis_map(repo_root / "basis-sets" / "cc-pVDZ", ["H", "O"])
    mol.cart = True
    mol.build()
    ...
    auxbasis = helper._load_basis_map(repo_root / "basis-sets" / "cc-pVDZ-RIFIT", ["H", "O"])
    pt = dfmp2_native.DFRMP2(mf, auxbasis=auxbasis)
    ecorr = pt.calculate_energy()
    mp2_time = time.perf_counter() - t1

    print(f"PYSCF_RHF_TOTAL_ENERGY {mf.e_tot:.14f}")
