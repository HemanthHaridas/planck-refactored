...
- **Post-HF** — RMP2 and UMP2 correlation energy corrections with natural orbital analysis; RCCSD for closed-shell RHF references; a teaching-oriented RCCSDT prototype for small systems; CASSCF and RASSCF multireference active-space calculations
- **RCCSD / RCCSDT** — coupled-cluster single-point energies for canonical RHF references. `RCCSD` uses an iterative spin-orbital amplitude solver; `RCCSDT` currently uses a determinant-space prototype intended for small teaching examples rather than large-scale production runs
...
| `correlation` | enum | `rmp2`, `ump2`, `ccsd`, `ccsdt`, `casscf`, `rasscf` | none | Post-HF method. `rmp2`/`ump2`: Møller-Plesset second-order correction. `ccsd`: restricted coupled cluster with singles and doubles for canonical RHF references. `ccsdt`: restricted coupled cluster with singles, doubles, and triples; currently implemented as a teaching-oriented determinant-space prototype for small RHF systems. `casscf`: Complete Active Space SCF (requires `nactele`, `nactorb`). `rasscf`: Restricted Active Space SCF (requires `nactele`, `nactorb`, `nras1`, `nras2`, `nras3`). |
